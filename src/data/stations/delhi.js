export const delhi = {
  name: "Delhi",
  country: "India",
  defaultConnectorSize: 60,
  auto: true,
  stations: {
    redLine: {
      color: "#e32726",
      rithala: {
        name: "Rithala",
      },
      rohiniWest: {
        name: "Rohini West",
      },
      rohiniEast: {
        name: "Rohini East",
      },
      pitampura: {
        name: "Pitampura",
      },
      kohatEnclave: {
        name: "Kohat Enclave",
      },
      netajiSubhashPlace: {
        name: "Netaji Subhash Place",
      },
      kanhiayanagar: {
        name: "Kanhiaya Nagar",
      },
      keshavPuram: {
        name: "Keshav Puram",
      },
      triNagar: {
        name: "Tri Nagar",
      },
      shastriNagar: {
        name: "Shastri Nagar",
      },
      inderlok: {
        name: "Inderlok",
      },
      tisHazari: {
        name: "Tis Hazari",
      },
      pulBangash: {
        name: "Pul Bangash",
      },
      pratapNagar: {
        name: "Pratap Nagar",
      },
      shastriPark: {
        name: "Shastri Park",
      },
      seelampur: {
        name: "Seelampur",
      },
      welcome: {
        name: "Welcome",
      },
      shahdara: {
        name: "Shahdara",
      },
      mansarovarPark: {
        name: "Mansarovar Park",
      },
      jhilmil: {
        name: "Jhilmil",
      },
      dilshadGarden: {
        name: "Dilshad Garden",
      },
      jhilmilColony: {
        name: "Jhilmil Colony",
      },
      mansarovarParkEast: {
        name: "Mansarovar Park East",
      },
      shivVihar: {
        name: "Shiv Vihar",
      },
      shaheedSthal: {
        name: "Shaheed Sthal New Bus Adda",
      },
      hindonRiver: {
        name: "Hindon River",
      },
      arthala: {
        name: "Arthala",
      },
      mohanNagar: {
        name: "Mohan Nagar",
      },
      shyamPark: {
        name: "Shyam Park",
      },
      majorMohitSharma: {
        name: "Major Mohit Sharma Rajendra Nagar",
      },
      rajBagh: {
        name: "Raj Bagh",
      },
      shaheedNagar: {
        name: "Shaheed Nagar",
      },
      kashmereGate: {
        name: "Kashmere Gate",
      },
    },
    yellowLine: {
      color: "#ffda2c",
      samaypurBadli: {
        name: "Samaypur Badli",
      },
      rohiniSectorEighteenNineteen: {
        name: "Rohini Sector 18 19",
      },
      haiderpurBadliMor: {
        name: "Haiderpur Badli Mor",
      },
      jahangirpuri: {
        name: "Jahangirpuri",
      },
      adarshNagar: {
        name: "Adarsh Nagar",
      },
      azadpur: {
        name: "Azadpur",
      },
      modelTown: {
        name: "Model Town",
      },
      gtuNagar: {
        name: "GTB Nagar",
      },
      vishwaVidyalaya: {
        name: "Vishwa Vidyalaya",
      },
      vidhanSabha: {
        name: "Vidhan Sabha",
      },
      civilLines: {
        name: "Civil Lines",
      },
      kashmereGateYellow: {
        name: "Kashmere Gate",
      },
      chandniChowk: {
        name: "Chandni Chowk",
      },
      chawriBazar: {
        name: "Chawri Bazar",
      },
      newDelhi: {
        name: "New Delhi",
      },
      rajivChowk: {
        name: "Rajiv Chowk",
      },
      patelChowk: {
        name: "Patel Chowk",
      },
      centralSecretariat: {
        name: "Central Secretariat",
      },
      udyogBhawan: {
        name: "Udyog Bhawan",
      },
      lokKalyanMarg: {
        name: "Lok Kalyan Marg",
      },
      jorBagh: {
        name: "Jor Bagh",
      },
      aiims: {
        name: "AIIMS",
      },
      greenPark: {
        name: "Green Park",
      },
      hauzKhas: {
        name: "Hauz Khas",
      },
      malaviyaNagar: {
        name: "Malviya Nagar",
      },
      saket: {
        name: "Saket",
      },
      qutubMinar: {
        name: "Qutub Minar",
      },
      chhatarpur: {
        name: "Chhatarpur",
      },
      sultanpur: {
        name: "Sultanpur",
      },
      ghitorni: {
        name: "Ghitorni",
      },
      arjanGarh: {
        name: "Arjan Garh",
      },
      guruDronacharya: {
        name: "Guru Dronacharya",
      },
      sikandarpur: {
        name: "Sikandarpur",
      },
      mgRoad: {
        name: "MG Road",
      },
      iffcoChowk: {
        name: "IFFCO Chowk",
      },
      hudaCityCentre: {
        name: "Huda City Centre",
      },
    },
    blueLine: {
      color: "#00549f",
      dwarkaSectorTwentyOne: {
        name: "Dwarka Sector 21",
      },
      dwarkaSectorEight: {
        name: "Dwarka Sector 8",
      },
      dwarkaSectorNine: {
        name: "Dwarka Sector 9",
      },
      dwarkaSectorTen: {
        name: "Dwarka Sector 10",
      },
      dwarkaSectorEleven: {
        name: "Dwarka Sector 11",
      },
      dwarka: {
        name: "Dwarka",
      },
      dwarkaMor: {
        name: "Dwarka Mor",
      },
      nawada: {
        name: "Nawada",
      },
      uttamNagarWest: {
        name: "Uttam Nagar West",
      },
      uttamNagarEast: {
        name: "Uttam Nagar East",
      },
      janakpuriWest: {
        name: "Janakpuri West",
      },
      janakpuriEast: {
        name: "Janakpuri East",
      },
      tilakNagar: {
        name: "Tilak Nagar",
      },
      subhashNagar: {
        name: "Subhash Nagar",
      },
      tagoreGarden: {
        name: "Tagore Garden",
      },
      rajouriGarden: {
        name: "Rajouri Garden",
      },
      rameshNagar: {
        name: "Ramesh Nagar",
      },
      motiNagar: {
        name: "Moti Nagar",
      },
      kirtiNagar: {
        name: "Kirti Nagar",
      },
      shadipur: {
        name: "Shadipur",
      },
      patelNagar: {
        name: "Patel Nagar",
      },
      rajendraPlace: {
        name: "Rajendra Place",
      },
      karolBagh: {
        name: "Karol Bagh",
      },
      jhandewalan: {
        name: "Jhandewalan",
      },
      rkAshramMarg: {
        name: "RK Ashram Marg",
      },
      rajivChowkBlue: {
        name: "Rajiv Chowk",
      },
      barakhambaRoad: {
        name: "Barakhamba Road",
      },
      mandiHouse: {
        name: "Mandi House",
      },
      supremeCourt: {
        name: "Supreme Court",
      },
      indraprastha: {
        name: "Indraprastha",
      },
      yamunaBank: {
        name: "Yamuna Bank",
      },
      branches: [
        {
          branchedStation: "Yamuna Bank",
          color: "#00549f",
          stations: {
            akshardham: {
              name: "Akshardham",
            },
            mayurViharPhaseOne: {
              name: "Mayur Vihar Phase 1",
            },
            mayurViharExtension: {
              name: "Mayur Vihar Extension",
            },
            newAshokNagar: {
              name: "New Ashok Nagar",
            },
            noidaSectorFifteen: {
              name: "Noida Sector 15",
            },
            noidaSectorSixteen: {
              name: "Noida Sector 16",
            },
            noidaSectorEighteen: {
              name: "Noida Sector 18",
            },
            botanicalGarden: {
              name: "Botanical Garden",
            },
            golfCourse: {
              name: "Golf Course",
            },
            noidaCityCentre: {
              name: "Noida City Centre",
            },
            noidaSectorThirtyFour: {
              name: "Noida Sector 34",
            },
            noidaSectorFiftyTwo: {
              name: "Noida Sector 52",
            },
            noidaSectorSixtyOne: {
              name: "Noida Sector 61",
            },
            noidaSectorFiftyNine: {
              name: "Noida Sector 59",
            },
            noidaSectorSixtyTwo: {
              name: "Noida Sector 62",
            },
            noidaElectronicCity: {
              name: "Noida Electronic City",
            },
          },
        },
        {
          branchedStation: "Yamuna Bank",
          color: "#00549f",
          stations: {
            laxmiNagar: {
              name: "Laxmi Nagar",
            },
            nirmanVihar: {
              name: "Nirman Vihar",
            },
            preetVihar: {
              name: "Preet Vihar",
            },
            karkarduma: {
              name: "Karkarduma",
            },
            anandVihar: {
              name: "Anand Vihar",
            },
            kaushambi: {
              name: "Kaushambi",
            },
            vaishali: {
              name: "Vaishali",
            },
          },
        },
      ],
    },
    greenLine: {
      color: "#00b24c",
      brigadierHoshiarSingh: {
        name: "Brigadier Hoshiar Singh",
      },
      mundka: {
        name: "Mundka",
      },
      ghevra: {
        name: "Ghevra",
      },
      tikriKalan: {
        name: "Tikri Kalan",
      },
      tikrіBorder: {
        name: "Tikri Border",
      },
      panditShreeramSharma: {
        name: "Pandit Shree Ram Sharma",
      },
      bahadurgarhCity: {
        name: "Bahadurgarh City",
      },
      ashokParkMain: {
        name: "Ashok Park Main",
      },
      branches: [
        {
          branchedStation: "Ashok Park Main",
          color: "#00b24c",
          stations: {
            satguruRamSinghMarg: {
              name: "Satguru Ram Singh Marg",
            },
            inderlokGreen: {
              name: "Inderlok",
            },
            kanhaiyaNagar: {
              name: "Kanhaiya Nagar",
            },
            keshavPuramGreen: {
              name: "Keshav Puram",
            },
            kirtiNagarGreen: {
              name: "Kirti Nagar",
            },
          },
        },
      ],
    },
    violetLine: {
      color: "#964b96",
      kashmereGateViolet: {
        name: "Kashmere Gate",
      },
      lalQuila: {
        name: "Lal Quila",
      },
      jamaМasjid: {
        name: "Jama Masjid",
      },
      delhiGate: {
        name: "Delhi Gate",
      },
      itоRailwayStation: {
        name: "ITO",
      },
      mandiHouseViolet: {
        name: "Mandi House",
      },
      janpathViolet: {
        name: "Janpath",
      },
      centralSecretariatViolet: {
        name: "Central Secretariat",
      },
      khanMarket: {
        name: "Khan Market",
      },
      jlnStadium: {
        name: "JLN Stadium",
      },
      jangpura: {
        name: "Jangpura",
      },
      lajpatNagar: {
        name: "Lajpat Nagar",
      },
      moolchand: {
        name: "Moolchand",
      },
      kailashColony: {
        name: "Kailash Colony",
      },
      nehruPlace: {
        name: "Nehru Place",
      },
      kalkajiMandir: {
        name: "Kalkaji Mandir",
      },
      govindpuriViolet: {
        name: "Govindpuri",
      },
      harkeshNagar: {
        name: "Harkesh Nagar Okhla",
      },
      jasolaVihar: {
        name: "Jasola Vihar Shaheen Bagh",
      },
      kalkајiMandir: {
        name: "Kalindi Kunj",
      },
      okhlaВirdSanctuary: {
        name: "Okhla Bird Sanctuary",
      },
      botanicalGardenViolet: {
        name: "Botanical Garden",
      },
      okhlaNsic: {
        name: "Okhla NSIC",
      },
      sukhdevVihar: {
        name: "Sukhdev Vihar",
      },
      jamiaМilliaIslamia: {
        name: "Jamia Millia Islamia",
      },
      jasolaApollo: {
        name: "Jasola Apollo",
      },
      tughlakabad: {
        name: "Tughlakabad",
      },
      mohanEstate: {
        name: "Mohan Estate",
      },
      badarpurBorder: {
        name: "Badarpur Border",
      },
      sarai: {
        name: "Sarai",
      },
      nhpcChowk: {
        name: "NHPC Chowk",
      },
      mujesar: {
        name: "Mujesar",
      },
      escortsMujesar: {
        name: "Escorts Mujesar",
      },
      rajaNaharSingh: {
        name: "Raja Nahar Singh Ballabhgarh",
      },
    },
    orangeLine: {
      color: "#f68712",
      newDelhiOrange: {
        name: "New Delhi",
      },
      shivajіStadium: {
        name: "Shivaji Stadium",
      },
      dhaulaKuan: {
        name: "Dhaula Kuan",
      },
      delhiAeroCityOrange: {
        name: "Delhi Aerocity",
      },
      airportTerminalThree: {
        name: "IGI Airport Terminal 3",
      },
      dwarkaSectorTwentyOne: {
        name: "Dwarka Sector 21",
      },
      yashobhoomiDwarkaSectorTwentyFive: {
        name: "Yashobhoomi Dwarka Sector 25",
      },
    },
    magentaLine: {
      color: "#d71b5a",
      janakpuriWestMagenta: {
        name: "Janakpuri West",
      },
      dabriMor: {
        name: "Dabri Mor Janakpuri South",
      },
      dashrathPuri: {
        name: "Dashrath Puri",
      },
      palamMagenta: {
        name: "Palam",
      },
      sadarBazar: {
        name: "Sadar Bazar Cantonment",
      },
      terminalOneMagenta: {
        name: "Terminal 1 IGI Airport",
      },
      shankarVihar: {
        name: "Shankar Vihar",
      },
      vasantVihar: {
        name: "Vasant Vihar",
      },
      munirka: {
        name: "Munirka",
      },
      rkPuram: {
        name: "RK Puram",
      },
      iitMagenta: {
        name: "IIT Delhi",
      },
      hauzKhasMagenta: {
        name: "Hauz Khas",
      },
      paschimVihar: {
        name: "Panchsheel Park",
      },
      chiragDelhi: {
        name: "Chirag Delhi",
      },
      greaterKailash: {
        name: "Greater Kailash",
      },
      nehruEnclaveMagenta: {
        name: "Nehru Enclave",
      },
      kalkajiMandirMagenta: {
        name: "Kalkaji Mandir",
      },
      okhla: {
        name: "Okhla",
      },
      jasolaViharShaheenBagh: {
        name: "Jasola Vihar Shaheen Bagh",
      },
      okhlaBirdSanctuaryMagenta: {
        name: "Okhla Bird Sanctuary",
      },
      kalindiKunjMagenta: {
        name: "Kalindi Kunj",
      },
      jasolaApolloMagenta: {
        name: "Jasola Apollo",
      },
      saritaVihar: {
        name: "Sarita Vihar",
      },
      mohanEstateMagenta: {
        name: "Mohan Estate",
      },
      tughlakabadMagenta: {
        name: "Tughlakabad",
      },
      botanicalGardenMagenta: {
        name: "Botanical Garden",
      },
    },
    pinkLine: {
      color: "#e895cd",
      maјlisPark: {
        name: "Majlis Park",
      },
      azadpurPink: {
        name: "Azadpur",
      },
      shalimarBagh: {
        name: "Shalimar Bagh",
      },
      netajiSubhashPlacePink: {
        name: "Netaji Subhash Place",
      },
      shakespeareSarani: {
        name: "Shakurpur",
      },
      kanhaiyaPuramPink: {
        name: "Kanhaiya Nagar",
      },
      keshaevPuramPink: {
        name: "Keshav Puram",
      },
      triNagarPink: {
        name: "Tri Nagar",
      },
      eastAzadNagar: {
        name: "East Azad Nagar",
      },
      westEnclavePink: {
        name: "Punjabi Bagh West",
      },
      ashokParkMainPink: {
        name: "Ashok Park Main",
      },
      satguruRamSingh: {
        name: "Satguru Ram Singh Marg",
      },
      mayapuri: {
        name: "Mayapuri",
      },
      narainаVihar: {
        name: "Naraina Vihar",
      },
      delhiCanttPink: {
        name: "Delhi Cantt",
      },
      durgabaiBesai: {
        name: "Durgabai Deshmukh South Campus",
      },
      sirVishweshwaraiah: {
        name: "Sir Vishweshwaraiah Moti Bagh",
      },
      bheekajіCamaPink: {
        name: "Bhikaji Cama Place",
      },
      sarojiniNagar: {
        name: "Sarojini Nagar",
      },
      dilliHaat: {
        name: "Dilli Haat INA",
      },
      southExtension: {
        name: "South Extension",
      },
      lajpatNagarPink: {
        name: "Lajpat Nagar",
      },
      vinobapuri: {
        name: "Vinobapuri",
      },
      ashramPink: {
        name: "Ashram",
      },
      hazratNizamuddin: {
        name: "Hazrat Nizamuddin",
      },
      mayurViharPocketOne: {
        name: "Mayur Vihar Pocket 1",
      },
      trilokpuri: {
        name: "Trilokpuri Sanjay Lake",
      },
      vinodNagarEast: {
        name: "Vinod Nagar East",
      },
      vinodNagarWest: {
        name: "Vinod Nagar West",
      },
      ipExtension: {
        name: "IP Extension",
      },
      anandViharPink: {
        name: "Anand Vihar",
      },
      karkardumaCourt: {
        name: "Karkarduma Court",
      },
      karkardumaPink: {
        name: "Karkarduma",
      },
      krishnaNagarPink: {
        name: "Krishna Nagar",
      },
      eastAzadNagarPink: {
        name: "East Azad Nagar",
      },
      welcomePink: {
        name: "Welcome",
      },
      jaffrabad: {
        name: "Jaffrabad",
      },
      maujpurBabarpur: {
        name: "Maujpur Babarpur",
      },
      gokulpurі: {
        name: "Gokulpuri",
      },
      johnnyPaul: {
        name: "Johri Enclave",
      },
      shivViharPink: {
        name: "Shiv Vihar",
      },
    },
    greyLine: {
      color: "#969696",
      dwarkaPink: {
        name: "Dwarka",
      },
      nangli: {
        name: "Nangli",
      },
      najafgarh: {
        name: "Najafgarh",
      },
      dhansaBusStand: {
        name: "Dhansa Bus Stand",
      },
    },
  },
};
