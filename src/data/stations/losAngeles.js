export const losAngeles = {
  name: "Los Angeles",
  country: "United States",
  defaultConnectorSize: 60,
  auto: true,
  stations: {
    lineABlue: {
      color: "#0072bc",
      pomonaNorth: {
        name: "Pomona North",
      },
      laVerneFairplex: {
        name: "La Verne/Fairplex",
      },
      sanDimas: {
        name: "San Dimas",
      },
      glendora: {
        name: "Glendora",
      },
      apuCitrusCollege: {
        name: "APU/Citrus College",
      },
      irwindale: {
        name: "Irwindale",
      },
      azusaDowntown: {
        name: "Azusa Downtown",
      },
      sierraMadreVilla: {
        name: "Sierra Madre Villa",
      },
      allenSouthPasadena: {
        name: "Allen (South Pasadena)",
      },
      lakeCaltech: {
        name: "Lake (Caltech)",
      },
      memorialPark: {
        name: "Memorial Park",
      },
      swJunction: {
        name: "SW Junction",
      },
      fillmore: {
        name: "Fillmore",
      },
      delMar: {
        name: "Del Mar",
      },
      mission: {
        name: "Mission",
      },
      southPasadena: {
        name: "South Pasadena",
      },
      lincolnCypress: {
        name: "Lincoln/Cypress",
      },
      heritageSquare: {
        name: "Heritage Square",
      },
      chinatown: {
        name: "Chinatown",
      },
      unionStationA: {
        name: "Union Station",
      },
      littleTokyoArtsDistrictA: {
        name: "Little Tokyo/Arts District",
      },
      picoA: {
        name: "Pico",
      },
      grandLattc: {
        name: "Grand/LATTC",
      },
      sanPedro: {
        name: "San Pedro",
      },
      washingtonBoulevard: {
        name: "Washington Boulevard",
      },
      vernon: {
        name: "Vernon",
      },
      slauson: {
        name: "Slauson",
      },
      florence: {
        name: "Florence",
      },
      firestone: {
        name: "Firestone",
      },
      grahamWatts: {
        name: "103rd/Graham",
      },
      wattsCollege: {
        name: "103rd/Watts College",
      },
      willowbrookRosaParksA: {
        name: "Willowbrook/Rosa Parks",
      },
      compton: {
        name: "Compton",
      },
      artesia: {
        name: "Artesia",
      },
      delAmo: {
        name: "Del Amo",
      },
      wardlow: {
        name: "Wardlow",
      },
      willow: {
        name: "Willow",
      },
      pacificCoast: {
        name: "Pacific Coast",
      },
      anaheim: {
        name: "Anaheim",
      },
      fifthStreet: {
        name: "5th Street",
      },
      firstStreet: {
        name: "1st Street",
      },
      pacificAvenue: {
        name: "Pacific Avenue",
      },
      downtownLongBeach: {
        name: "Downtown Long Beach",
      },
    },
    lineBRed: {
      color: "#e22e27",
      northHollywood: {
        name: "North Hollywood",
      },
      universalCityHollywood: {
        name: "Universal City/Hollywood",
      },
      hollywoodHighland: {
        name: "Hollywood/Highland",
      },
      hollywoodVine: {
        name: "Hollywood/Vine",
      },
      hollywoodWestern: {
        name: "Hollywood/Western",
      },
      vermontSunset: {
        name: "Vermont/Sunset",
      },
      vermontSantaMonica: {
        name: "Vermont/Santa Monica",
      },
      vermontBeverly: {
        name: "Vermont/Beverly",
      },
      wilshireVermontB: {
        name: "Wilshire/Vermont",
      },
      westlakeMacarthurPark: {
        name: "Westlake/MacArthur Park",
      },
      pershingSquare: {
        name: "Pershing Square",
      },
      civicCenterGrandPark: {
        name: "Civic Center/Grand Park",
      },
      unionStationB: {
        name: "Union Station",
      },
    },
    lineCGreen: {
      color: "#58a738",
      redondoBeach: {
        name: "Redondo Beach",
      },
      douglas: {
        name: "Douglas",
      },
      elSegundo: {
        name: "El Segundo",
      },
      mariposa: {
        name: "Mariposa",
      },
      aviationCenturyC: {
        name: "Aviation/Century",
      },
      laxMetroTransitCenterC: {
        name: "LAX/Metro Transit Center",
      },
      hawthorneLennox: {
        name: "Hawthorne/Lennox",
      },
      crenshawImperial: {
        name: "Crenshaw/Imperial",
      },
      vermontAthens: {
        name: "Vermont/Athens",
      },
      harbourFreeway: {
        name: "Harbour Freeway",
      },
      willowbrookRosaParksC: {
        name: "Willowbrook/Rosa Parks",
      },
      longBeach: {
        name: "Long Beach",
      },
      lakewood: {
        name: "Lakewood",
      },
      norwalk: {
        name: "Norwalk",
      },
    },
    lineDPurple: {
      color: "#a05da5",
      wilshireWestern: {
        name: "Wilshire/Western",
      },
      wilshireNormandie: {
        name: "Wilshire/Normandie",
      },
      wilshireVermontD: {
        name: "Wilshire/Vermont",
      },
      westlakeMacarthurParkD: {
        name: "Westlake/MacArthur Park",
      },
      pershingSquareD: {
        name: "Pershing Square",
      },
      civicCenterGrandParkD: {
        name: "Civic Center/Grand Park",
      },
      unionStationD: {
        name: "Union Station",
      },
      wilshireLaBrea: {
        name: "Wilshire/La Brea",
      },
      wilshireFairfax: {
        name: "Wilshire/Fairfax",
      },
      wilshireLaCienega: {
        name: "Wilshire/La Cienega",
      },
      wilshireRodeo: {
        name: "Wilshire/Rodeo",
      },
      centuryCity: {
        name: "Century City",
      },
      westwood: {
        name: "Westwood",
      },
      westwoodVaHospital: {
        name: "Westwood/VA Hospital",
      },
    },
    lineEExpo: {
      color: "#f1ba3a",
      littleTokyoArtsDistrictE: {
        name: "Little Tokyo/Arts District",
      },
      historicSouthCentralArts: {
        name: "Historic South Central/Arts",
      },
      mariachiPlaza: {
        name: "Mariachi Plaza",
      },
      indianaAtlantic: {
        name: "Indiana/Atlantic",
      },
      maravillaSanGabriel: {
        name: "Maravilla/San Gabriel",
      },
      eastLosAngelesCivicCenter: {
        name: "East Los Angeles Civic Center",
      },
      atlantic: {
        name: "Atlantic",
      },
      picoE: {
        name: "Pico",
      },
      lattc: {
        name: "LATTC",
      },
      seventhMetroCenter: {
        name: "7th St/Metro Center",
      },
      expoCrenshaw: {
        name: "Expo/Crenshaw",
      },
      expoWestern: {
        name: "Expo/Western",
      },
      expoVermont: {
        name: "Expo/Vermont",
      },
      expoParkUsc: {
        name: "Expo Park/USC",
      },
      jeffersonVermont: {
        name: "Jefferson/Vermont",
      },
      jeffersonUsc: {
        name: "Jefferson/USC",
      },
      expoLaBrea: {
        name: "Expo/La Brea",
      },
      farmdale: {
        name: "Farmdale",
      },
      expoCulver: {
        name: "Expo/Culver",
      },
      palms: {
        name: "Palms",
      },
      westwoodRanchoPark: {
        name: "Westwood/Rancho Park",
      },
      sepulveda: {
        name: "Sepulveda",
      },
      expoSepulveda: {
        name: "Expo/Sepulveda",
      },
      bundy: {
        name: "Bundy",
      },
      bergamot: {
        name: "26th/Bergamot",
      },
      smcCollege: {
        name: "17th/SMC",
      },
      downtownSantaMonica: {
        name: "Downtown Santa Monica",
      },
    },
    lineKCrenshaw: {
      color: "#d42e87",
      laxMetroTransitCenterK: {
        name: "LAX/Metro Transit Center",
      },
      aviationCenturyK: {
        name: "Aviation/Century",
      },
      westchesterVeterans: {
        name: "Westchester/Veterans",
      },
      downtownInglewood: {
        name: "Downtown Inglewood",
      },
      westsideParkway: {
        name: "Westside Parkway",
      },
      leimertPark: {
        name: "Leimert Park",
      },
      mlkJr: {
        name: "MLK Jr",
      },
      expoCrenshaW: {
        name: "Expo/Crenshaw",
      },
    },
  },
};
