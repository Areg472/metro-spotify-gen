export const chicago = {
  name: "Chicago",
  country: "United States",
  defaultConnectorSize: 60,
  auto: true,
  stations: {
    redLine: {
      color: "#c60c30",
      howard: {
        name: "Howard",
      },
      jarvis: {
        name: "Jarvis",
      },
      morse: {
        name: "Morse",
      },
      loyola: {
        name: "Loyola",
      },
      granville: {
        name: "Granville",
      },
      thorndale: {
        name: "Thorndale",
      },
      brynMawr: {
        name: "Bryn Mawr",
      },
      berwyn: {
        name: "Berwyn",
      },
      argyle: {
        name: "Argyle",
      },
      lawrence: {
        name: "Lawrence",
      },
      wilson: {
        name: "Wilson",
      },
      sheridan: {
        name: "Sheridan",
      },
      addison: {
        name: "Addison",
      },
      belmont: {
        name: "Belmont",
      },
      fullerton: {
        name: "Fullerton",
      },
      northClybourn: {
        name: "North/Clybourn",
      },
      clarkDivision: {
        name: "Clark/Division",
      },
      chicago: {
        name: "Chicago",
      },
      grandRedLine: {
        name: "Grand",
      },
      lakeRedLine: {
        name: "Lake",
      },
      monroeRedLine: {
        name: "Monroe",
      },
      jacksonRedLine: {
        name: "Jackson",
      },
      harrison: {
        name: "Harrison",
      },
      roosevelt: {
        name: "Roosevelt",
      },
      cermakChinatown: {
        name: "Cermak-Chinatown",
      },
      sox35th: {
        name: "Sox-35th",
      },
      fortySeventhRedLine: {
        name: "47th",
      },
      garfield: {
        name: "Garfield",
      },
      sixtyThird: {
        name: "63rd",
      },
      sixtyNinth: {
        name: "69th",
      },
      seventyNinth: {
        name: "79th",
      },
      eightySeventh: {
        name: "87th",
      },
      ninetyFifthNintyFifthDanRyan: {
        name: "95th/Dan Ryan",
      },
    },
    blueLine: {
      color: "#00a1de",
      ohare: {
        name: "O'Hare",
      },
      rosemont: {
        name: "Rosemont",
      },
      cumberland: {
        name: "Cumberland",
      },
      harlemOhare: {
        name: "Harlem",
      },
      jeffersonPark: {
        name: "Jefferson Park",
      },
      montrose: {
        name: "Montrose",
      },
      irvingPark: {
        name: "Irving Park",
      },
      addisonBlueLine: {
        name: "Addison",
      },
      belmontBlueLine: {
        name: "Belmont",
      },
      loganSquare: {
        name: "Logan Square",
      },
      californiaBlue: {
        name: "California",
      },
      westernBlueLine: {
        name: "Western",
      },
      damen: {
        name: "Damen",
      },
      divisionBlueLine: {
        name: "Division",
      },
      chicagoBlueLine: {
        name: "Chicago",
      },
      grandBlueLine: {
        name: "Grand",
      },
      clarkLake: {
        name: "Clark/Lake",
      },
      washingtonBlueLine: {
        name: "Washington",
      },
      monroeBlueLine: {
        name: "Monroe",
      },
      jacksonBlueLine: {
        name: "Jackson",
      },
      lasalleVanBuren: {
        name: "LaSalle/Van Buren",
      },
      clintonBlueLine: {
        name: "Clinton",
      },
      uicHalsted: {
        name: "UIC-Halsted",
      },
      racine: {
        name: "Racine",
      },
      illinoisMedicalDistrict: {
        name: "Illinois Medical District",
      },
      westernForestPark: {
        name: "Western (Forest Park)",
      },
      kedziеHoman: {
        name: "Kedzie-Homan",
      },
      pulaskiBlueLine: {
        name: "Pulaski",
      },
      cicero: {
        name: "Cicero",
      },
      austinBlueLine: {
        name: "Austin",
      },
      oakPark: {
        name: "Oak Park",
      },
      harlemForestPark: {
        name: "Harlem (Forest Park)",
      },
      forestPark: {
        name: "Forest Park",
      },
    },
    brownLine: {
      color: "#62361b",
      kimball: {
        name: "Kimball",
      },
      kedzie: {
        name: "Kedzie",
      },
      francisco: {
        name: "Francisco",
      },
      rockwell: {
        name: "Rockwell",
      },
      westernBrownLine: {
        name: "Western",
      },
      damenBrownLine: {
        name: "Damen",
      },
      montroseBrownLine: {
        name: "Montrose",
      },
      irvingParkBrownLine: {
        name: "Irving Park",
      },
      addisonBrownLine: {
        name: "Addison",
      },
      paulina: {
        name: "Paulina",
      },
      southport: {
        name: "Southport",
      },
      belmontBrownLine: {
        name: "Belmont",
      },
      wellingtonBrownLine: {
        name: "Wellington",
      },
      diversey: {
        name: "Diversey",
      },
      fullertonBrownLine: {
        name: "Fullerton",
      },
      armitage: {
        name: "Armitage",
      },
      sedgwick: {
        name: "Sedgwick",
      },
      chicagoArmitage: {
        name: "Chicago",
      },
      merchandiseMart: {
        name: "Merchandise Mart",
      },
      washingtonWells: {
        name: "Washington/Wells",
      },
      quincy: {
        name: "Quincy",
      },
      lasalleVanBurenBrown: {
        name: "LaSalle/Van Buren",
      },
      libraryVanBuren: {
        name: "Library-Van Buren",
      },
      adamsWabash: {
        name: "Adams/Wabash",
      },
      madisonWabash: {
        name: "Madison/Wabash",
      },
      stateLake: {
        name: "State/Lake",
      },
      clarkLakeBrown: {
        name: "Clark/Lake",
      },
    },
    greenLine: {
      color: "#009b3a",
      harlemLake: {
        name: "Harlem/Lake",
      },
      oakParkGreen: {
        name: "Oak Park",
      },
      ridgeland: {
        name: "Ridgeland",
      },
      austinGreen: {
        name: "Austin",
      },
      central: {
        name: "Central",
      },
      laramie: {
        name: "Laramie",
      },
      ciceroGreen: {
        name: "Cicero",
      },
      pulaskiGreen: {
        name: "Pulaski",
      },
      conservatoryCentralParkDrive: {
        name: "Conservatory-Central Park Drive",
      },
      kedzieGreen: {
        name: "Kedzie",
      },
      californiaGreen: {
        name: "California",
      },
      ashland: {
        name: "Ashland",
      },
      morganGreen: {
        name: "Morgan",
      },
      clintonGreen: {
        name: "Clinton",
      },
      clarkLakeGreen: {
        name: "Clark/Lake",
      },
      stateLakeGreen: {
        name: "State/Lake",
      },
      washingtonWabash: {
        name: "Washington/Wabash",
      },
      adamsWabashGreen: {
        name: "Adams/Wabash",
      },
      rooseveltGreen: {
        name: "Roosevelt",
      },
      branches: [
        {
          branchedStation: "Roosevelt",
          color: "#009b3a",
          stations: {
            cermak: {
              name: "Cermak-McCormick Place",
            },
            thirtyFifthBronzeville: {
              name: "35th-Bronzeville-IIT",
            },
            indianaGreen: {
              name: "Indiana",
            },
            fortyThirdGreen: {
              name: "43rd",
            },
            fortySeventhGreen: {
              name: "47th",
            },
            fiftyFirstGreen: {
              name: "51st",
            },
            garfieldGreen: {
              name: "Garfield",
            },
            kingDrive: {
              name: "King Drive",
            },
            cottageGrove: {
              name: "Cottage Grove",
            },
          },
        },
        {
          branchedStation: "Roosevelt",
          color: "#009b3a",
          stations: {
            cermakMcCormickPlace: {
              name: "Cermak-McCormick Place",
            },
            thirtyFifthBronzeville: {
              name: "35th-Bronzeville-IIT",
            },
            indianaAshland: {
              name: "Indiana",
            },
            halstedAshland: {
              name: "Halsted",
            },
            ashlandAshland: {
              name: "Ashland/63rd",
            },
          },
        },
      ],
    },
    orangeLine: {
      color: "#f9461c",
      midway: {
        name: "Midway",
      },
      pulaskiOrange: {
        name: "Pulaski",
      },
      kedzieOrange: {
        name: "Kedzie",
      },
      westernOrange: {
        name: "Western",
      },
      thirtyFifthArcher: {
        name: "35th/Archer",
      },
      ashlandOrange: {
        name: "Ashland",
      },
      halsted: {
        name: "Halsted",
      },
      rooseveltOrange: {
        name: "Roosevelt",
      },
      libraryVanBurenOrange: {
        name: "Library-Van Buren",
      },
      lasalleVanBurenOrange: {
        name: "LaSalle/Van Buren",
      },
      quincyOrange: {
        name: "Quincy",
      },
      washingtonWellsOrange: {
        name: "Washington/Wells",
      },
      clarkLakeOrange: {
        name: "Clark/Lake",
      },
      stateLakeOrange: {
        name: "State/Lake",
      },
      madisonWabashOrange: {
        name: "Madison/Wabash",
      },
      adamsWabashOrange: {
        name: "Adams/Wabash",
      },
    },
    pinkLine: {
      color: "#e27ea6",
      fiftyFourthCermak: {
        name: "54th/Cermak",
      },
      ciceroPink: {
        name: "Cicero",
      },
      kostner: {
        name: "Kostner",
      },
      pulaskiPink: {
        name: "Pulaski",
      },
      centralPark: {
        name: "Central Park",
      },
      kedziePink: {
        name: "Kedzie",
      },
      californiaPink: {
        name: "California",
      },
      westernPink: {
        name: "Western",
      },
      damenPink: {
        name: "Damen",
      },
      eighteenth: {
        name: "18th",
      },
      polkPink: {
        name: "Polk",
      },
      ashlandPink: {
        name: "Ashland",
      },
      morganPink: {
        name: "Morgan",
      },
      clintonPink: {
        name: "Clinton",
      },
      clarkLakePink: {
        name: "Clark/Lake",
      },
      stateLakePink: {
        name: "State/Lake",
      },
      washingtonWabashPink: {
        name: "Washington/Wabash",
      },
      adamsWabashPink: {
        name: "Adams/Wabash",
      },
    },
    purpleLine: {
      color: "#522398",
      linden: {
        name: "Linden",
      },
      centralPurple: {
        name: "Central",
      },
      noyes: {
        name: "Noyes",
      },
      fosterPurple: {
        name: "Foster",
      },
      davis: {
        name: "Davis",
      },
      dempster: {
        name: "Dempster",
      },
      mainPurple: {
        name: "Main",
      },
      southBoulevard: {
        name: "South Boulevard",
      },
      howardPurple: {
        name: "Howard",
      },
      jarvisPurple: {
        name: "Jarvis",
      },
      morsePurple: {
        name: "Morse",
      },
      loyolaPurple: {
        name: "Loyola",
      },
      granvillePurple: {
        name: "Granville",
      },
      thorndalePurple: {
        name: "Thorndale",
      },
      brynMawrPurple: {
        name: "Bryn Mawr",
      },
      berwynPurple: {
        name: "Berwyn",
      },
      argylePurple: {
        name: "Argyle",
      },
      lawrencePurple: {
        name: "Lawrence",
      },
      wilsonPurple: {
        name: "Wilson",
      },
      sheridanPurple: {
        name: "Sheridan",
      },
      addisonPurple: {
        name: "Addison",
      },
      belmontPurple: {
        name: "Belmont",
      },
      fullertonPurple: {
        name: "Fullerton",
      },
    },
    yellowLine: {
      color: "#f9e300",
      dempsterSkokie: {
        name: "Dempster-Skokie",
      },
      oakton: {
        name: "Oakton-Skokie",
      },
      howardYellow: {
        name: "Howard",
      },
    },
  },
};
